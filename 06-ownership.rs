...
} // The Vec does not get disposed because `borrow` does not own the value

// A function can borrow mutably by taking a mutable reference `&mut`
fn change(v: &mut Vec<i32>) {
    println!("I am updating the Vec: {:?}", v);
    v.push(4);
} // The Vec does not get disposed because `change` does not own the value